...
with tab2:
    ...
    c1.metric("Accuracy",  f"{metricas_train['Accuracy']:.4f}")
    c2.metric("Precision", f"{metricas_train['Precision']:.4f}")
    c3.metric("Recall",    f"{metricas_train['Recall']:.4f}")
    c4.metric("F1-Score",  f"{metricas_train['F1']:.4f}")
    c5.metric("ROC AUC",   f"{metricas_train['ROC_AUC']:.4f}")
    col_roc, col_imp = st.columns(2)
    with col_roc:
        st.markdown('<div class="section-header">Curva ROC (Train)</div>', unsafe_allow_html=True)
        fig_roc, ax_roc = plt.subplots(figsize=(5, 4))
        ax_roc.plot(roc_data["fpr"], roc_data["tpr"], color=PALETTE["accent"], lw=2, label=f"AUC = {metricas_train['ROC_AUC']:.4f}")
